...
Begin VB.UserControl oLine 
   ...
   ScaleHeight     =   600
   ScaleWidth      =   840
   ToolboxBitmap   =   "xLine.ctx":0000
   Windowless      =   -1  'True
End
Attribute VB_Name = "oLine"
...
Private WithEvents m_ConnectedFrom As oGraph.oPicture
Attribute m_ConnectedFrom.VB_VarHelpID = -1

Private WithEvents m_ConnectedTo As oGraph.oPicture
...
Private m_LayereLineType As eLineType
...
Public Property Get ObjectType() As eObjectType
    ObjectType = oTLine
End Property

Public Property Get ConnectedFrom() As oGraph.oPicture
    Set ConnectedFrom = m_ConnectedFrom
End Property

Public Property Set ConnectedFrom(ByRef pLayedObject As oGraph.oPicture)
...
Public Property Get ConnectedTo() As oGraph.oPicture
    Set ConnectedTo = m_ConnectedTo
End Property

Public Property Set ConnectedTo(ByRef pLayedObject As oGraph.oPicture)
...
Public Property Get LayereLineType() As eLineType
    LayereLineType = m_LayereLineType
End Property

Public Property Let LayereLineType(ByVal New_LayereLineType As eLineType)
    m_LayereLineType = New_LayereLineType
    PropertyChanged "LayereLineType"
End Property
'--------------------------Properties End-------------------------------
...
Private Sub m_ConnectedFrom_MouseMove(ByVal Key As String, ByVal Button As Integer, ByVal Shift As Integer, ByVal X As Double, ByVal Y As Double)
    ...
    If m_MovingLinePoints And Button = vbLeftButton Then
        If m_TempFromUbound >= 0 Then
            pX = m_TempFromPx1(m_TempFromUbound)
            pY = m_TempFromPy1(m_TempFromUbound)
        Else
...
Private Sub MarkPointForDebug(ByVal pColor As Colors, X As Double, Y As Double)
    m_oConvasPicBox.Line (X * 15, Y * 15)-((X + 4) * 15, (Y + 4) * 15), vbRed, B
End Sub
    ...
    If m_Selected Then
        If Not m_RealinedOnce Then
            ...
            If Not (ShiftX = ShiftY And ShiftX = 0) Then
                ...
                Next
                m_PointCount = CalculatePointCount(m_Points)
                Call CreatePath
...
Public Sub Paint()
    ...
    If m_Visible Then
        ...
        If Not m_Use3D Then
            ...
            Call GdipDrawPath(m_IXGraphics.GetoGraphicsHandle, m_Pen, m_Path)
        Else
            For pIndex = 0 To pPointCount '- 1
                If pIndex > 0 Then
                    If Abs(pLinePoints(pIndex).X - pLinePoints(pIndex - 1).X) < m_LineWidth Then 'Verticle
...
Private Sub DrawCircle(ByVal pOnlyCreate As Boolean, ByRef pPoint As LINE_POINT)
    ...
    Dim poGraphics As Long
    
    
    If m_Selected Then
        pColour = sSelected
    Else
        Select Case m_LayereLineType
            Case OnTCompletion:
                pColour = sBlue
            Case OnTFail:
                pColour = sRed
    ...
    Else
        poGraphics = m_IXGraphics.GetoGraphicsHandle
        ...
        If m_BrushCircle Then
            GdipFillPath poGraphics, m_BrushCircle, m_PathCircle
        End If
    
        GdipCreatePen1 pStartColor, pPenWidth, UnitPixel, pPen
        If pPen Then
            GdipDrawPath poGraphics, pPen, m_PathCircle
            GdipDeletePen pPen
        End If
...
Private Sub DrawSegment(ByVal Index As Single, ByVal pOnlyCreate As Boolean, ByVal pDirectonCode As DirectionCode, ByRef pPtStart As LINE_POINT, ByRef pPtEnd As LINE_POINT, ByVal DrawJoint As Boolean, ByVal pPreviousDirCode As DirectionCode)
    Dim pPenWidth As Long
    Dim phDC As Long, poGraphics As Long
    Dim pPath As Long
    Dim pPathII As Long
    Dim pStartColor As Long
    Dim pEndColor As Long
    ...
    Else
        ...
        If pBrush Then
            ...
            GdipFillPath poGraphics, pBrush, m_3DSegmentPath(Index)
            GdipDeleteBrush pBrush
        End If
            
        If pPen Then
            GdipDrawPath poGraphics, pPen, m_3DSegmentPath(Index)
        End If
        ...
        If DrawJoint Then
            ...
            If pblnPointDrawn Then
                ...
                GdipSetLineGammaCorrection pBrush, True
                If pBrush Then
                    GdipFillPath poGraphics, pBrush, pPath
                    GdipDeleteBrush pBrush
                End If
                
            '    GdipDrawPath poGraphics, pPen, pPath
                GdipDeletePath pPath
            ...
            Else
                ...
                If pBrush Then
                    Call GdipFillRectangle(poGraphics, pBrush, pRect.Left, pRect.Top, pRect.Width, pRect.Height)
                    GdipDeleteBrush pBrush
                End If
            End If
    End If
  ...
  '  If pPoint.x = 0 Then Stop
    ...
    End Select
        ...
        If m_BrushArrow Then
             GdipFillPath poGraphics, m_BrushArrow, m_PathArrow
             GdipDeleteBrush m_BrushArrow
         End If
        
        GdipCreatePen1 pStartColor, pPenWidth, UnitPixel, pPen
        If pPen Then
            GdipDrawPath poGraphics, pPen, m_PathArrow
            GdipDeletePen pPen
        End If
...
Private Sub DrawTempLinesFromToPoint(ByVal X As Double, ByVal Y As Double, ByVal XAct As Double, ByVal YAct As Double)
    ...
    For pIndex = 0 To m_PointCount - 2
        ...
        If Abs(px1 - px2) < m_LineWidth Then
            pSegOrient = 1  'Ver
        Else
            pSegOrient = 0
        End If
...
Private Function DrawFinalArrow(ByVal X1 As Double, ByVal Y1 As Double, ByVal X2 As Double, ByVal Y2 As Double) As LINE_POINT
    ...
    If X1 = X2 And Y1 = Y2 And X1 = 0 And Y1 = 0 Then
        If m_Visible Then m_oConvasPicBox.Line (pArrowPoint(0).X * 15, pArrowPoint(0).Y * 15)-(pArrowPoint(1).X * 15, pArrowPoint(1).Y * 15)
                ...
                Select Case pIndex:
                    ...
                    Case pPointCount:
                        ...
                        ElseIf pDirectionCode = D2U Then
                            ...
                            Call DrawSegment(pIndex, True, pDirectionCode, pLinePoints(pIndex - 1), ptPoint, False, 0)
                        End If
                        If pIndex = 1 And pPointCount = 1 Then
...
Private Sub CreatePen(ByVal pPenWidth As Single)
    ...
    Select Case m_LayereLineType
        Case eLineType.OnTCompletion:
            pColour = Black
            pBackColour = Blue
        Case eLineType.OnTFail:
            pColour = Black
            pBackColour = Red
        Case eLineType.OnTSuccess:
            pColour = Black
            pBackColour = Green
    End Select
    If m_Selected Then
...
Private Function ValidatePoints(ByRef pPoints() As LINE_POINT) As LINE_POINT()
    Dim pValidPoints() As LINE_POINT
    Dim pIndex As Single
    Dim pUbound As Single
    Dim pValidIndex As Single
    pUbound = UBound(pPoints)
    
    'For pIndex = 0 To pUbound
        'Debug.Print pPoints(pIndex).X, pPoints(pIndex).Y
    'Next
    
    ReDim Preserve pValidPoints(pValidIndex)
    Let pValidPoints(pValidIndex) = pPoints(0)
    pValidIndex = 1
    For pIndex = 1 To pUbound
        If Abs(pPoints(pIndex - 1).X - pPoints(pIndex).X) < m_LineWidth Or Abs(pPoints(pIndex - 1).Y - pPoints(pIndex).Y) < m_LineWidth Then
            ReDim Preserve pValidPoints(pValidIndex)
            pValidPoints(pValidIndex) = pPoints(pIndex)
            pValidIndex = pValidIndex + 1
        Else
'            Stop
        End If
    Next
    ValidatePoints = pValidPoints
End Function


Private Function RemoveUnwantedPoints(ByRef pPoints() As LINE_POINT) As LINE_POINT()
    Dim pIndex As Single
    Dim pUbound As Single
    Dim pLinePoints As LINE_POINT
    Dim pXShift As Double
    Dim pYShift As Double
    Dim pInnerLoop As Single
    Dim pStrPointToRemove As String
    Dim pNoOfPointsToRemove As Single
    Dim pNewPoints() As LINE_POINT
    Dim pChrs() As String
    ...
    pNoOfPointsToRemove = 0
    pStrPointToRemove = ""
    For pIndex = 1 To pUbound - 1
        If Abs(pPoints(pIndex).X - pPoints(pIndex + 1).X) = 0 Then  'Line is Verticle
            If Abs(pPoints(pIndex).Y - pPoints(pIndex + 1).Y) < m_LineWidth Then
                pYShift = pPoints(pIndex).Y - pPoints(pIndex + 1).Y
                pStrPointToRemove = "'" & pIndex & "','" & pIndex + 1 & "',"
                pNoOfPointsToRemove = pNoOfPointsToRemove + 2
                For pInnerLoop = pIndex To pUbound '- 1
                  pPoints(pInnerLoop).Y = pPoints(pInnerLoop).Y + pYShift
                Next
            End If
        Else    'Line is Horizontal
            If Abs(pPoints(pIndex).X - pPoints(pIndex + 1).X) < m_LineWidth Then
                pXShift = pPoints(pIndex).X - pPoints(pIndex + 1).X
                pStrPointToRemove = "'" & pIndex & "','" & pIndex + 1 & "',"
                pNoOfPointsToRemove = pNoOfPointsToRemove + 2
                For pInnerLoop = pIndex To pUbound '- 1
                  pPoints(pInnerLoop).X = pPoints(pInnerLoop).X + pXShift
                Next
            End If
        End If
    Next
    
    If pNoOfPointsToRemove > 0 Then
        pStrPointToRemove = Left(pStrPointToRemove, Len(pStrPointToRemove) - 1)
        pChrs = Split(pStrPointToRemove, ",")
        pcount = UBound(pChrs) + 1
        If pcount <> pNoOfPointsToRemove Then
            pNoOfPointsToRemove = pcount
        End If
    End If
    
    If pNoOfPointsToRemove > 0 Then
        If Abs(pUbound - pNoOfPointsToRemove) = 0 Then
            ReDim pNewPoints(pUbound - pNoOfPointsToRemove + 1)
        Else
            ReDim pNewPoints(pUbound - pNoOfPointsToRemove)
        End If
        pInnerLoop = 0
        For pIndex = 0 To pUbound
            If pIndex = 0 Then
                Let pNewPoints(pInnerLoop) = pPoints(pIndex)
                pInnerLoop = pInnerLoop + 1
            Else
                If InStr(1, pStrPointToRemove, "'" & pIndex & "'") <= 0 Then
            ...
            End If
        Next
        RemoveUnwantedPoints = pNewPoints
    Else
        RemoveUnwantedPoints = pPoints
    End If
    Exit Function
Errortrap:
    RemoveUnwantedPoints = pPoints
...
Public Sub SetPoints(ByRef pPoints() As LINE_POINT)
    Dim pValidPoints() As LINE_POINT
    Dim pResultPoints() As LINE_POINT
    Dim pPointCount As Single
    pValidPoints = ValidatePoints(pPoints)
    pResultPoints = RemoveUnwantedPoints(pValidPoints)
    pPointCount = CalculatePointCount(pResultPoints)
    If pPointCount >= 2 Then
        m_PointCount = pPointCount
        m_Points = pResultPoints
...
Private Function CalculatePointCount(ByRef pPoints() As LINE_POINT) As Single
    On Error GoTo Errortrap
    CalculatePointCount = UBound(pPoints) + 1
    'If CalculatePointCount <= 1 Then Stop
    Exit Function
Errortrap:
    CalculatePointCount = 0
End Function
        ...
        If m_Region <> 0 And Not m_blnPointMoving And Not m_MovingLinePoints And Button = 0 Then
            ...
            If pResult = 1 Then
               ...
               If Shift <> vbCtrlMask Then
                    ...
                    m_oConvasPicBox.MousePointer = GetPointer(X / 15, Y / 15)
               Else
                    m_oConvasPicBox.ToolTipText = ""
...
Private Function DrawLineTo(ByRef pLineIndex As Single, ByVal X1 As Double, ByVal Y1 As Double, ByVal X2 As Double, ByVal Y2 As Double, Optional ByVal ErasePresious As Boolean = True, Optional ByVal LastSegment As Boolean) As LINE_POINT()
        ...
        If X1 = X2 And Y1 = Y2 And X1 = Y1 And X1 = 0 Then
            ...
            If pLineIndex = -1 And m_TempToUbound > 0 Then
                For pIndex = 0 To m_TempToUbound
                    If m_Visible Then m_oConvasPicBox.Line (m_TempToPx1(pIndex) * 15, m_TempToPy1(pIndex) * 15)-(m_TempToPx2(pIndex) * 15, m_TempToPy2(pIndex) * 15)
                Next
                Erase m_TempToPx1
                Erase m_TempToPy1
                Erase m_TempToPx2
            ...
            If LastSegment Then
                For pIndex = pLineIndex To m_TempToUbound
                    If m_Visible Then m_oConvasPicBox.Line (m_TempToPx1(pLineIndex) * 15, m_TempToPy1(pLineIndex) * 15)-(m_TempToPx2(pLineIndex) * 15, m_TempToPy2(pLineIndex) * 15)
                Next
            ...
            If m_TempToUbound >= 0 Then
                For pLineIndex = 0 To m_TempToUbound
                        If pLineIndex > 0 Then
                            If Abs(pPoints(pLineIndex - 1).X - m_TempToPx1(pLineIndex)) < m_LineWidth And Abs(pPoints(pLineIndex - 1).Y - m_TempToPy1(pLineIndex)) < m_LineWidth Then
                                If pIndex > 1 Then
                                    pIndex = pIndex - 1
                                End If
                            Else
        ...
        If ErasePresious And m_Visible Then m_oConvasPicBox.Line (m_TempToPx1(pLineIndex) * 15, m_TempToPy1(pLineIndex) * 15)-(m_TempToPx2(pLineIndex) * 15, m_TempToPy2(pLineIndex) * 15)
...
Private Function DrawLineFrom(ByRef pLineIndex As Single, ByVal X1 As Double, ByVal Y1 As Double, ByVal X2 As Double, ByVal Y2 As Double, Optional ByVal ErasePresious As Boolean = True, Optional ByVal LastSegment As Boolean) As LINE_POINT()
        ...
        If X1 = X2 And Y1 = Y2 And X1 = Y1 And X1 = 0 Then
            ...
            If pLineIndex = -1 And m_TempFromUbound > 0 Then
                For pIndex = 0 To m_TempFromUbound
                 If m_Visible Then m_oConvasPicBox.Line (m_TempFromPx1(pIndex) * 15, m_TempFromPy1(pIndex) * 15)-(m_TempFromPx2(pIndex) * 15, m_TempFromPy2(pIndex) * 15)
                Next
                Erase m_TempFromPx1
                Erase m_TempFromPy1
                Erase m_TempFromPx2
            ...
            If LastSegment Then
                For pIndex = pLineIndex To m_TempFromUbound
                  If m_Visible Then m_oConvasPicBox.Line (m_TempFromPx1(pLineIndex) * 15, m_TempFromPy1(pLineIndex) * 15)-(m_TempFromPx2(pLineIndex) * 15, m_TempFromPy2(pLineIndex) * 15)
                Next
            ...
            If m_TempFromUbound >= 0 Then
                For pLineIndex = 0 To m_TempFromUbound
                        If pLineIndex > 0 Then
                            If Abs(pPoints(pLineIndex - 1).X - m_TempFromPx1(pLineIndex)) < m_LineWidth And Abs(pPoints(pLineIndex - 1).Y - m_TempFromPy1(pLineIndex)) < m_LineWidth Then
                                If pIndex > 1 Then
                                    pIndex = pIndex - 1
                                End If
                            Else
        ...
        m_oConvasPicBox.DrawMode = DrawModeConstants.vbInvert
        If ErasePresious And m_Visible Then m_oConvasPicBox.Line (m_TempFromPx1(pLineIndex) * 15, m_TempFromPy1(pLineIndex) * 15)-(m_TempFromPx2(pLineIndex) * 15, m_TempFromPy2(pLineIndex) * 15)
        ...
        If m_Visible Then m_oConvasPicBox.Line (X1 * 15, Y1 * 15)-(X2 * 15, Y2 * 15)
        pLineIndex = pLineIndex + 1
        'Debug.Print "Line Drawn ------------------------------------------------------"
        'For pIndex = 0 To m_TempFromUbound
        '  Debug.Print "Line (", m_NodeKey, ")", pLineIndex, ":", m_TempFromPx1(pIndex), m_TempFromPy1(pIndex), " To ", m_TempFromPx2(pIndex), m_TempFromPy2(pIndex)
        'Next
End Function
...
Private Sub ThreeSegmentLine(ByVal X As Double, ByVal Y As Double, Optional FromPoint As Boolean = False, Optional ByVal XAct As Double, Optional ByVal YAct As Double)
    ...
    Dim pLineIndex As Single
        ...
        pIndex = pIndex + 1
        If Abs(px1 - px2) < m_LineWidth Or Abs(py1 - py2) < m_LineWidth Then
            
            'Stop
        ElseIf Abs(px1 - px2) > Abs(py1 - py2) Then
            'Debug.Print "3 State 1"
            ReDim Preserve pPoints(pIndex)
...
Private Sub m_oConvasPicBox_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If m_Visible Then
        ...
        If m_blnPointMoving Then
            ...
            Me.SetPoints pPoints
            Me.Paint
            LSet pMaxExtend = GetMaxExtend
            RaiseEvent MaxExtend(pMaxExtend.X * 15, pMaxExtend.Y * 15)
            m_blnPointMoving = False
        End If
    End If
End Sub

Private Function GetMaxExtend() As POINTAPI
Dim pIndex As Single
Dim pX As Single
Dim pY As Single

For pIndex = 0 To m_PointCount - 1
    If pX < m_Points(pIndex).X Then
        pX = m_Points(pIndex).X
    End If
    
    If pY < m_Points(pIndex).Y Then
        pY = m_Points(pIndex).Y
    End If
Next

GetMaxExtend.X = pX + m_LineWidth
GetMaxExtend.Y = pY + m_LineWidth
End Function
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    m_LayereLineType = PropBag.ReadProperty("LayereLineType", eLineType.OnTSuccess)
...
'--------------------------Standard Functions-----------------------------------
Public Sub GetCordinates(ByRef CenX As Double, ByRef CenY As Double, ByRef oWidth As Double, ByRef oHight As Double, Optional ByVal ReLSetValues As Boolean)
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    Call PropBag.WriteProperty("LayereLineType", m_LayereLineType, eLineType.OnTSuccess)
...
Public Sub ScaleRegion(ByVal ZoomFactor As Double)
    ...
    Dim pMaxExtend As POINTAPI
    For pIndex = 0 To m_PointCount - 1
        m_Points(pIndex).X = m_Points(pIndex).X * ZoomFactor
        m_Points(pIndex).Y = m_Points(pIndex).Y * ZoomFactor
    Next
    
    m_XEnd = m_XEnd * ZoomFactor
    m_YEnd = m_YEnd * ZoomFactor
    m_XStart = m_XStart * ZoomFactor
    m_YStart = m_YStart * ZoomFactor
    m_LineWidth = m_LineWidth * ZoomFactor
    Call CreatePath
    Call CreatePen(m_LineWidth)
    LSet pMaxExtend = GetMaxExtend
    RaiseEvent MaxExtend(pMaxExtend.X * 15, pMaxExtend.Y * 15)
