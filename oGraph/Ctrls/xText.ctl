...
Begin VB.UserControl oText 
   ...
   PropertyPages   =   "xText.ctx":0000
   ScaleHeight     =   1080
   ScaleWidth      =   2220
   Windowless      =   -1  'True
End
Attribute VB_Name = "oText"
...
Public Property Let tHeight(ByVal pHeight As Single)
    ...
    If Not m_IXGraphics Is Nothing Then
        Call CreatePath
        Call CreateRegion
    End If
...
Public Sub Paint()
    ...
    If m_Visible Then
        ...
        If m_IXGraphics Is Nothing Then Exit Sub
        If m_Selected Then
            pWidth = 3
            pColor = DarkBlue
        Else
        ...
        Else
            Call GdipFillPath(m_IXGraphics.GetoGraphicsHandle, pTextBrush, m_PathBack)
        End If
        Call GdipDrawPath(m_IXGraphics.GetoGraphicsHandle, pTextPen, m_Path)
        ...
        'Call GdipFillRegion(m_IXGraphics.GetoGraphicsHandle, pTextBrush, m_Region)
...
Private Sub m_oConvasPicBox_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If m_Visible Then
        If m_Region <> 0 And Not m_blnPointMoving And Button = 0 Then
            Call GdipIsVisibleRegionPoint(m_Region, X / 15, Y / 15, m_IXGraphics.GetoGraphicsHandle, pResult)
            If pResult = 1 Then
               Call GdipIsVisibleRegionPoint(m_RegionOuter, X / 15, Y / 15, m_IXGraphics.GetoGraphicsHandle, pResult)
               ...
               If pResult = 1 Then
                    ...
                    If Shift <> vbCtrlMask Then
                          If m_oConvasPicBox.Tag = m_NodeKey Then
                                ...
                                m_oConvasPicBox.MousePointer = GetPointer(X, Y)
                          End If
                    Else
                         m_oConvasPicBox.ToolTipText = ""
...
Private Sub m_oConvasPicBox_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If m_Visible Then
        ...
        If m_blnPointMoving Then
            ...
            RaiseEvent MaxExtend(m_CentreX + m_Width / 2 * 15, m_CentreY + m_Height / 2 * 15)
